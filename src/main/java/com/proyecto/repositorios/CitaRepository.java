...
public interface CitaRepository extends JpaRepository<Cita, Integer> {
	...
	String sqlBuscarMedicoCabecera = "Select m from Medico as m join m.listaCitas as medicocitas "
			+ "join medicocitas.paciente as p where p.nSS= :nSS and m.especialidad like 'Atencion primaria'";
	...
	List<Cita> buscarCitaXPacienteYHora(@Param("nss") String nSS, @Param("fecha") Date fecha);

	@Query(value = "Select c from Cita c where c.medico.nLicencia = :nLicencia and c.fHoraCita = :fecha")
	List<Cita> buscarCitaXMedicoYHora(@Param("nLicencia") String nLicencia, @Param("fecha") Date fecha);
